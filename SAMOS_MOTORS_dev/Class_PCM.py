...
print("line 63 PCM",local_dir)
parent_dir = str(path.parent)  
print("line 65 parent dir",parent_dir) 
...
#        print('v1:',bin(int(string)).lstrip('0b'))
    ...
    # Position B4
            if position == 'B4':
                current_steps = self.query_current_step_counts('FW2')
                current_steps = self.extract_steps_from_return_string(current_steps)
                print('1. current_steps:',current_steps, 'should be',str(self.FW2_counts_pos4))
                if current_steps != str(self.FW2_counts_pos4):
                    time.sleep(2)
                    s.sendall(b'~@,9600_8N1T2000,/2e4R\n')
                    current_steps = self.query_current_step_counts('FW2')
                    current_steps = self.extract_steps_from_return_string(current_steps)
                    print('2. current_steps:',current_steps, 'should be',str(self.FW2_counts_pos4))
                    while current_steps != str(self.FW2_counts_pos4):
                          time.sleep(2)
                          s.sendall(b'~@,9600_8N1T2000,/2e4R\n')
                          current_steps = self.query_current_step_counts('FW2')
                          current_steps = self.extract_steps_from_return_string(current_steps)
                          print('3. current_steps:',current_steps, 'should be',str(self.FW2_counts_pos4))
                self.write_status()          
                return position,current_steps
    #
    # Position B5
