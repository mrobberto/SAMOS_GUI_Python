...
def echo_client():
    ...
    HOST = '10.0.0.179'#127.0.0.1'  # The server's hostname or IP address
    PORT = 1000#65432        # The port used by the server
...
# 
# =============================================================================
def initialize_filter_wheel(FW):
    import socket
...
# ~@,9600_8N1T2000,/1s0m23l23h0j32V4000v2500n0P0P100z0M500e1R
        if FW == "FW1":
            s.sendall(b'~@,9600_8N1T2000,/1s0m23l23h0j32V4000v2500n0P0P100z0M500e1R\n')

# WHEEL 2
# Initial drive settings (velocity, steps ratio, current), plus homing routine — this runs automatically on power up.
# ~@,9600_8N1T2000,/2s0m23l23h0j32V4000v2500n0P0P100z0M500e1R
        if FW == "FW2":
            s.sendall(b'~@,9600_8N1T2000,/2s0m23l23h0j32V4000v2500n0P0P100z0M500e1R\n')
...
"""
def stored_filter_wheel_procedures():
    import socket
...
#         
# =============================================================================
def move_filter_wheel_(position):
    import socket
...
# Position A1
        if position == 'A1':
            current_steps = query_current_step_counts('FW1')
            if current_steps != 46667:
                s.sendall(b'~@,9600_8N1T2000,/1e1R\n')
                while current_steps != 46667:
                    current_steps = timed_query_current_count_monitor('FW1')
            # check sensor status  
...
# Position B1
        if position == 'B1':
            current_steps = query_current_step_counts('FW2')
            if current_steps != 46667:
                s.sendall(b'~@,9600_8N1T2000,/2e1R\n')
                while current_steps != 46667:
                    current_steps = timed_query_current_count_monitor('FW2')
            return position,current_steps       
# 
# Position B2
        if position == 'B2':
...
# 
# =============================================================================
def query_current_step_counts(FW):
    import socket
...
def timed_query_current_count_monitor(FW):
    import threading
    threading.Timer(1.0, query_current_step_counts(FW)).start()
...
#
# Init (A)
        s.sendall(b'~@,9600_8N1T2000,/3s0m23l23h0j4V7000v2500n2f1Z100000000R\n')

# Init (B)
        s.sendall(b'~@,9600_8N1T2000,/4s0m23l23h0j4V7000v2500n2f1Z100000000R\n')
...
def stored_grism_rails_procedures():
    import socket
